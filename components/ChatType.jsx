  ...
  fetch("/facebook/message-blocks/store", {
    ...
    .then((res) => {
      ...
      if (countCompleteSequence == total) {
        ...
        // if (url.indexOf("facebook/setting-messages")) {
        ...
        let url = window.location.href;
        var idSettingMessage = url.substring(url.lastIndexOf("/") + 1);
        window.location.href = "/facebook/setting-messages/config/" + idSettingMessage;
      }
    });
}
...
function CreateChatButton() {
  ...
  function removeOldBlocks() {
    ...
    if (message) {
      fetch("/facebook/message-settings/remove-block/" + message.id, {
        method: "GET",
      })
        .then((res) => {
          return res.json();
  ...
  function createBlock() {
    ...
    for (let i = 0; i < itemList.length; i++) {
      ...
      data.append("message_type", "broadcast_sequence");
      ...
      if (type == "text") {
        ...
        if (itemList[i].content.value) {
          if (itemList[i].content.value.value) {
            data.append("button_name", itemList[i].content.value.buttonName);
            data.append("button_type", itemList[i].content.value.buttonType);
            data.append("button_value", itemList[i].content.value.value);
          } else {
            window.window.alertDanger("Tồn tại button chưa setting giá trị");
  ...
  return (
    <button type="button" onClick={() => removeOldBlocks()} className="page-header__next-btn basic__btn">
      Tiếp theo
    </button>
  );
}
