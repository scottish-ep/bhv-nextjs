...
function getCurrentUserPageAccessToken(page_id) {
  ...
  if (localStorage.getItem("facebook-portal-choosed-pages-from-user")) {
    pages_ = JSON.parse(localStorage.getItem("facebook-portal-choosed-pages-from-user"));
  }
...
function PortalChatBox({ user, settings }) {
  ...
  useEffect(
    function () {
      ...
      channel.bind("message-event", function (response) {
        ...
        if (response.message && response.user.user_id === currentChatUserId && response.isMessage) {
  ...
  function onClickSendImage() {
    ...
    if (tab == "chat" || actionType == "private_reply") {
      ...
      data.append(
        "image",
        document.getElementById("btn__upload-media-comment").getAttribute("data-src"),
      );
      var element = document.querySelector(".fa__reply.active");
      if (element) {
        let commentId = element.getAttribute("data-id");
  ...
  function addImage(e) {
    ...
    if (tab == "chat" || actionType == "private_reply") {
      ...
      document.getElementById("btn__upload-media").innerHTML = "Uploading...";
      document.getElementById("btn__upload-media-private").style.display = "block";
      document.getElementById("btn__upload-media-private").disabled = true;
      document.getElementById("btn__upload-media-private").innerHTML = "Uploading...";
    } else if (tab == "comment") {
      document.getElementById("btn__upload-media-comment").style.display = "block";
      document.getElementById("btn__upload-media-comment").disabled = true;
      document.getElementById("btn__upload-media-comment").innerHTML = "Uploading...";
    }
    if (user) {
      // TODO: upload image
        ...
        fetch("/file/normal-upload", {
          ...
          .then((res) => {
            ...
            if (countUploaded == filesLength) {
              if (tab == "chat" || actionType == "private_reply") {
                document.getElementById("btn__upload-media").disabled = false;
                document.getElementById("btn__upload-media").innerHTML = "Send";
                document.getElementById("btn__upload-media").setAttribute("data-src", res.result);
                document.getElementById("btn__upload-media-private").disabled = false;
                document.getElementById("btn__upload-media-private").innerHTML = "Send";
                document
                  .getElementById("btn__upload-media-private")
                  .setAttribute("data-src", res.result);
              } else if (tab == "comment") {
                document.getElementById("btn__upload-media-comment").disabled = false;
                document.getElementById("btn__upload-media-comment").innerHTML = "Send";
                document
  ...
  function sendVideo({ offset, size, myVideo, data, uniqueId, extension, currentId }) {
  ...
  function readVideo({ offset, myVideo, size, uniqueId, extension, currentId }) {
    if (size <= offset) {
      return;
    }
      ...
      if (user && media.isSelected) {
        ...
        if (tab == "chat" || actionType == "private_reply") {
          ...
          sendFileToFacebook(
            ...
            null,
            true,
            media.type == "image" ? media.url : media.url.substring(media.url.lastIndexOf("/") + 1),
          );
          scrollToBottomChatView();
        } else if (tab == "comment") {
          let data = new FormData();
    ...
    <div className="portal-chat">
      ...
      <div className="portal-chat__content-container">
        <div className="portal-chat__header">
          <div  
            id="trigger-reset" className="d-none" 
            onClick={() => {
              console.log('checked');
              setIsLoading(true);
            ...
            {messageList.map(function (message, idx) {
              ...
              if (message.sender == "me") {
                ...
                if (idx != lastCustomerIndexMessage) {
                  if (idx + 1 < messageList.length && messageList[idx + 1].sender == "customer") {
                    showImage = false;
                  }
                } else {
                  showImage = true;
        ...
        <div
          ...
          className={"portal-chat__tab portal-chat__comment " + (tab === "comment" ? "active" : "")}
          onScroll={function (e) {
        ...
        >
          <div className="card-body">
            ...
            {isLoading && user && <div className="relative"><LoadingComponent size={24} color="#865fe9" /></div>}
          </div>
        </div>
      </div>
      ...
      >
          ...
          <div className="upload__image-wrapper">
            ...
            {!!listImageUrls.length &&
              listImageUrls.map(function (imageUrl, idx) {
                return <img src={imageUrl} key={idx} className="upload__portal-image"></img>;
              })}
          </div>
        )}
        ...
        <form id="form__chat" data-prevent_default="true" onSubmit={sendMessage}>
          <textarea id="submit__chat" type="text" name="content" placeholder="Type a message" />
      ...
      >
        ...
        <form id="form__chat_element" data-prevent_default="true" onSubmit={sendReplyComment}>
          <textarea
            id="submit__comment"
            type="text"
            name="content"
