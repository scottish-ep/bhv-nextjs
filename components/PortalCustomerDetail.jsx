...
function PortalCustomerDetail({ user, updateUserSupporterList }) {
  ...
  function handleUpdateTagList(e) {
    ...
    let fullTagSet = new Set(tagifyRef.current.value.map(({ value }) => value).concat(tagList));
      ...
      <div className="portal-customer-detail--wrapper">
          ...
          <div className="portal-customer-detail__user-info">
            ...
            <p className="user__order-status">
              <span className="user__success-order">
                Tổng đơn hàng <strong>{user.customerDetail.totalOrderCount}</strong>
              </span>
              <span className="user__failed-order">
                Tổng đơn hủy <strong>{user.customerDetail.transferOrderCount}</strong>
              </span>
            </p>

            {user.systemTag && <p className="portal-customer-detail__user-tag">{user.systemTag}</p>}
          </div>
        </div>
        ...
        <div className="portal-customer-detail__user_action card__border">
          ...
          <div className="portal-customer-detail__user-section">
            ...
            <form className="form__user_info_tag" onSubmit={handleUpdateTagList}>
          ...
          <div>
            ...
            <button type="button" onClick={supportUser} className="portal-customer-detail__btn2">
              Nhận chăm sóc
            </button>

            <button type="button" onClick={unseenMessage} className="portal-customer-detail__btn2">
