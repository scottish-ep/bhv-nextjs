  ...
  fetch("/facebook/message-blocks/store", {
    ...
    .then((res) => {
      ...
      if (countCompleteSequence == total) {
        ...
        // if (url.indexOf("facebook/setting-auto-inbox")) {
        ...
        let url = window.location.href;
        var idSettingMessage = url.substring(url.lastIndexOf("/") + 1);
        window.location.href = "/facebook/setting-auto-comment";
      }
    });
}
...
function CreateChatButton() {
  ...
  function removeOldBlocks() {
    ...
    if (message) {
      fetch("/facebook/setting-auto-inbox/remove-block/" + message.id, {
        method: "GET",
      })
        .then((res) => {
          return res.json();
  ...
  function createBlock() {
    ...
    for (let i = 0; i < itemList.length; i++) {
      ...
      data.append("message_type", "private_reply");
