...
const ModalPayDetail = (props: ModalPayDetailProps) => {
	const {isVisible, isEdit, title, iconClose= "Đóng", onClose, onOpen, content, titleBody, time, deal, method, status, id} = props;
				...
				<div className="w-full">
					<div className="w-full flex flex-col p-[12px] bg-white rounded-lg">
						<div className="flex justify-between mb-[12px] items-center mb-[24px]">
							<p className="text-medium font-medium text-[#2E2D3D]">Tên Giao dịch</p>
							{isEdit ? <Input width={460} placeholder="Nhập nội dung" /> : 
						...
						<div className="flex justify-between mb-[12px] items-center mb-[24px]">
							<p className="text-medium font-medium text-[#2E2D3D]">Loại hoá đơn</p>
								<Radio.Group className="w-[420px]">
								<div className="mr-[95px]">
									<Radio value="Phiếu thu" onClick={() => isEdit ? true : false} >Phiếu thu</Radio>
								</div>
								<Radio value="Phiếu chi" onClick={() => isEdit ? true : false}>Phiếu chi</Radio>
							</Radio.Group>
						</div>
						<div className="flex justify-between mb-[12px] items-center mb-[24px]">
							<p className="text-medium font-medium text-[#2E2D3D]">Ngày tạo</p>
							{isEdit ? <DatePicker width={460}  /> : 
								<div className="w-[150px]">{pay.date}</div> 
							}
						</div>
						<div className="flex justify-between mb-[12px] items-center mb-[24px] items-center">
							<p className="text-medium font-medium text-[#2E2D3D]">Nhân viên xử lý</p>
							{isEdit ? <Select width={460} placeholder="--" options={pay.name} /> : 
						...
						<div className="flex justify-between mb-[12px] items-center mb-[24px] items-center">
							...
							{isEdit ? <Select width={460} placeholder="--" options={pay.store} /> : 
								<div className="w-[440px] flex justify-between p-[10px] items-center">
									Nguyễn Văn A
								<Icon icon="arrow-down-1" size={14}/>
							</div> 
					...
					<div className="mt-[12px] flex flex-col w-full bg-white rounded-lg p-[12px]">
						<div className="flex justify-between mb-[12px] items-center mb-[24px]">
							<p className="text-medium font-medium text-[#2E2D3D]">Người nhận</p>
							{isEdit ? <Input width={460} placeholder="Nhập họ tên" /> : 
								<div className="w-[420px]">{pay.receiver}</div> 
							}
						</div>
						<div className="flex justify-between mb-[12px] items-center mb-[24px]">
							<p className="text-medium font-medium text-[#2E2D3D]">Số điện thoại</p>
							{isEdit ? <Input width={460} placeholder="Nhập SĐT" /> : 
								<div className="w-[420px]">{pay.phone}</div> 
							}
						</div>
						<div className="flex justify-between mb-[12px] items-center mb-[24px] items-center">
							<p className="text-medium font-medium text-[#2E2D3D]">Hình thức</p>
							{isEdit ? <Select width={460} placeholder="Chọn" options={pay.method} /> : 
								<div className="w-[440px] flex justify-between p-[10px] items-center">
									Nguyễn Văn A
								<Icon icon="arrow-down-1" size={14}/>
							</div> 
					...
					<div className="mt-[12px] w-full flex flex-col bg-white p-[12px] rounded-lg">
						<div className="w-full flex flex-col p-[12px]">
							<div className="flex w-full">
									...
									<p className="mb-[8px] text-meium font-medium w-[30%]">Hình ảnh</p>
							</div>
					</div>
					</div>
					<div className="w-full bg-white mt-[12px] rounded-lg p-[12px]">
						<div className="flex justify-between items-center">
								<p className="text-medium font-medium text-[#2E2D3D]">Trạng thái</p>
									<Radio.Group className="w-[440px]">
									<div className="mr-[60px]">
										<Radio value="Chờ duyệt" onClick={() => isEdit ? true : false} >Chờ duyệt</Radio>
									</div>
									<div className="mr-[60px]">
										<Radio value="Đã duyệt" onClick={() => isEdit ? true : false}>Đã duyệt</Radio>
									</div>
									<Radio value="Đã nhận" onClick={() => isEdit ? true : false}>Đã nhận</Radio>
								</Radio.Group>
							</div>
					</div>
					<div className="w-full flex justify-end mt-[32px]">
							<Button variant="outlined" className="mr-[12px] font-bold bg-white" width={305} height={44} text="HUỶ BỎ"/>
							<Button
								variant="outlined"
								width={305}
							...
							<Button variant="secondary"  width={305} height={44}  text="LƯU (F12)"/>
					</div>
